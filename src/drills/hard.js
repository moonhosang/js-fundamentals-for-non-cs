...
;(function () {
  ...
  H["5"] = {
    ...
    problems: [
      ...
      {"label":"🔨 선언·조건 전체","ask":"짝수면 true를 돌려주는 isEven을 '통째로' 선언하라","code":"____\nprint(isEven(4) === true && isEven(3) === false)","expect":"true","answer":"function isEven(n) { return n % 2 === 0 }","hint":"나머지 연산 n % 2 === 0","explain":"<code>n % 2 === 0</code>은 2로 나눈 나머지가 0(=짝수)이면 true를 <b>값으로</b> 낳는다 → 그대로 return. <code>function isEven(n) { return n % 2 === 0 }</code>.","see":"5-4"},
      {"label":"🔨 선언·화살표로","ask":"n의 2배를 돌려주는 dbl을 '화살표 함수'로 선언하라","code":"____\nprint(dbl(5) === 10)","expect":"true","answer":"const dbl = (n) => n * 2","hint":"const 이름 = (매개변수) => 식","explain":"화살표 함수 선언: <code>const dbl = (n) => n * 2</code>(한 줄이면 return 생략). 보통 함수 <code>function dbl(n){return n*2}</code>와 완전히 같다.","see":"5-7"}
    ],
  }
  H["6"] = {
