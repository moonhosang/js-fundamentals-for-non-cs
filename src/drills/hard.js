...
;(function () {
  ...
  H["objprim"] = {
    ...
    problems: [
      ...
      {"label":"함수 인자·객체","ask":"f(o)가 p.v = 9 하면 o.v는?","code":"function f(p) { p.v = 9 }\nlet o = { v: 1 }\nf(o)\nprint((o.v) === ____)","expect":"true","answer":"9","hint":"인자로 주소 복사 → p와 o는 같은 봉투","explain":"<b>객체=주소 복사(공유)</b> — 인자로 넘길 때도 <b>주소를 복사</b>해 전달 → 함수 안 p와 밖 o가 <b>같은 봉투</b>. <code>p.v=9</code>가 그걸 고쳐 <b>o.v도 9</b>. (원시를 넘겼다면 값 복사(독립)라 원본 안전)","mem":{"title":"o.v도 9 — 인자로 주소가 복사돼 p와 o가 같은 봉투","stackLabel":"📇 이름표 장부","code":["let o = { v: 1 }","f(o)   // p = o","p.v = 9"],"steps":[{"line":1,"stack":[{"name":"main","slots":[{"name":"o","ref":"h1"}]},{"name":"f","slots":[{"name":"p","ref":"h1"}]}],"heap":{"h1":{"fields":[{"key":"v","value":"1"}]}},"note":"호출 → <b>주소를 복사</b> → <code>p</code>도 <b>같은 h1</b>을 가리킨다."},{"line":2,"stack":[{"name":"main","slots":[{"name":"o","ref":"h1"}]},{"name":"f","slots":[{"name":"p","ref":"h1"}]}],"heap":{"h1":{"fields":[{"key":"v","value":"9","bad":true}]}},"note":"<code>p.v = 9</code> → h1을 고침 → <b>o.v도 9</b>."},{"line":1,"stack":[{"name":"main","slots":[{"name":"o","ref":"h1"}]}],"heap":{"h1":{"fields":[{"key":"v","value":"9","bad":true}]}},"returning":{"value":"undefined","discarded":true},"note":"f는 <b>return이 없어 undefined를 통로로</b>(💨 버려짐) 내보내고 <b>pop</b> — p 사라짐. 그래도 <b>o.v는 9</b>(같은 봉투)."}]}},
      {"label":"함수·재할당 함정","ask":"함수 안에서 매개변수를 새 객체로 재할당하면 — 원본 u.hp는?","code":"let u = { hp: 100 }\nfunction reset(p) { p = { hp: 0 } }\nreset(u)\nprint((u.hp) === ____)","expect":"true","answer":"100","hint":"재할당은 매개변수 화살표만 새 봉투로 — 원본 그대로","explain":"<code>p = { hp: 0 }</code>은 매개변수 <b>p의 화살표만 새 봉투(h2)</b>로 옮긴다 — 바깥 <code>u</code>는 <b>옛 봉투 h1 그대로</b>. 그래서 <b>u.hp는 100</b>. (변경 <code>p.hp=0</code>과 정반대 — 재할당은 원본을 못 건드린다.)","mem":{"title":"왜 u.hp는 100 — 재할당은 매개변수 화살표만 옮긴다","stackLabel":"📇 이름표 장부","code":["let u = { hp: 100 }","reset(u) → p = u (주소 공유)","p = { hp: 0 }  (새 봉투)"],"steps":[{"line":0,"stack":[{"name":"main","slots":[{"name":"u","ref":"h1"}]}],"heap":{"h1":{"fields":[{"key":"hp","value":"100"}]}},"note":"u는 힙 봉투 h1."},{"line":1,"stack":[{"name":"main","slots":[{"name":"u","ref":"h1"}]},{"name":"reset","slots":[{"name":"p","ref":"h1"}]}],"heap":{"h1":{"fields":[{"key":"hp","value":"100"}]}},"note":"<code>reset(u)</code> → p에 h1 주소 복사 → 같은 봉투."},{"line":2,"stack":[{"name":"main","slots":[{"name":"u","ref":"h1"}]},{"name":"reset","slots":[{"name":"p","ref":"h2","bad":true}]}],"heap":{"h1":{"fields":[{"key":"hp","value":"100"}]},"h2":{"fields":[{"key":"hp","value":"0"}]}},"note":"<code>p = { hp: 0 }</code>은 <b>새 봉투 h2</b>를 만들고 <b>p의 화살표만</b> h2로 옮긴다. u는 <b>여전히 h1(hp:100)</b> — 원본 안전."},{"line":2,"stack":[{"name":"main","slots":[{"name":"u","ref":"h1"}]}],"heap":{"h1":{"fields":[{"key":"hp","value":"100"}]}},"returning":{"value":"undefined","discarded":true},"note":"reset은 <b>undefined를 통로로</b>(💨 버려짐) 내보내고 <b>pop</b> — p(h2를 가리키던)도 사라지고 h2(새 봉투)는 아무도 안 가리켜 GC 대상. <b>u는 h1(hp:100) 그대로</b> — 재할당은 원본을 못 건드린다."}]}}
    ],
  }
})()
