...
;(function () {
  ...
  N["5"] = {
    ...
    problems: [
      ...
      {"label":"print는 못 담음","ask":"return이 없는 함수의 결과 r은?","code":"function f() { let msg = \"안녕\" }\nlet r = f()\nprint((r === undefined) === ____)","expect":"true","answer":"true","hint":"return 없으면 undefined","explain":"<code>f</code>는 안에서 계산만 하고 <b>return이 없다</b> → undefined를 돌려준다 → r은 undefined. <b>print도 똑같다</b> — 화면에 찍을 뿐 값을 안 돌려줘 담을 수 없다(return≠print)."},
  ...
  N["objprim"] = {
    ...
    problems: [
      ...
      {"label":"함수·객체 변경","ask":"객체를 함수에 넘겨 함수 안에서 p.hp를 0으로 바꾸면 — 원본 u.hp는?","code":"let u = { hp: 100 }\nfunction hurt(p) { p.hp = 0 }\nhurt(u)\nprint((u.hp) === ____)","expect":"true","answer":"0","hint":"객체는 주소 복사=공유 — 같은 봉투","explain":"함수에 넘길 때 <b>주소가 복사=공유</b> → 매개변수 p와 u는 <b>같은 봉투</b>. <code>p.hp = 0</code>이 그 봉투를 고치니 <b>u.hp도 0</b>. <b>객체=주소 복사(공유)</b>.","mem":{"title":"왜 u.hp도 0 — 객체는 주소가 전달돼 같은 봉투","stackLabel":"📇 이름표 장부","code":["let u = { hp: 100 }","hurt(u) → p = u (주소 공유)","p.hp = 0  (함수 안)"],"steps":[{"line":0,"stack":[{"name":"main","slots":[{"name":"u","ref":"h1"}]}],"heap":{"h1":{"fields":[{"key":"hp","value":"100"}]}},"note":"u는 힙 봉투 h1을 가리킨다."},{"line":1,"stack":[{"name":"main","slots":[{"name":"u","ref":"h1"}]},{"name":"hurt","slots":[{"name":"p","ref":"h1"}]}],"heap":{"h1":{"fields":[{"key":"hp","value":"100"}]}},"note":"<code>hurt(u)</code> → p에 <b>주소 h1 복사</b> → p와 u는 <b>같은 봉투</b>(공유)."},{"line":2,"stack":[{"name":"main","slots":[{"name":"u","ref":"h1"}]},{"name":"hurt","slots":[{"name":"p","ref":"h1"}]}],"heap":{"h1":{"fields":[{"key":"hp","value":"0","bad":true}]}},"note":"<code>p.hp = 0</code>이 h1을 고침 → <b>u.hp도 0</b>."},{"line":2,"stack":[{"name":"main","slots":[{"name":"u","ref":"h1"}]}],"heap":{"h1":{"fields":[{"key":"hp","value":"0","bad":true}]}},"returning":{"value":"undefined","discarded":true},"note":"hurt는 <b>return이 없어 undefined를 통로로</b>(💨 버려짐) 내보내고 <b>pop</b> — p 사라짐. 그래도 <b>u.hp는 0</b>(같은 봉투 변경이 남음)."}]}},
  ...
  N["destructuring"] = {
    pattern: "🟡 보통 · 구조분해 변형 — 기본값·별칭·중첩·스왑 (=== true 면 정답)",
    problems: [
      {label:"기본값", ask:"없을 때 기본값은?", code:"let { c = 5 } = { a: 1 }\nprint(c === ____)", expect:"true", answer:"5", hint:"키가 없으면 = 뒤 값을 쓴다", explain:"<code>{ c = 5 }</code>는 c 키가 <b>없으면</b> 기본값 5를 쓴다 → 5.", see:"destructuring"},
      {label:"기본값 무시", ask:"값이 있으면 기본값은?", code:"let { a = 9 } = { a: 1 }\nprint(a === ____)", expect:"true", answer:"1", hint:"있으면 그 값, 기본값 무시", explain:"a 키가 <b>있으면</b>(1) 기본값 9는 <b>안 쓴다</b> → 1. 기본값은 '없을 때만'."},
      {label:"별칭", ask:"a를 x로 별칭 받으면 x는?", code:"let { a: x } = { a: 7 }\nprint(x === ____)", expect:"true", answer:"7", hint:"{ 키: 새이름 }", explain:"<code>{ a: x }</code>는 a 값을 <b>다른 이름 x</b>로 받는다 → x = 7. (여기 : 는 '별칭', 객체 만들 때의 : 와 다름)"},
      {label:"중첩", ask:"중첩 객체 안 q는?", code:"let { p: { q } } = { p: { q: 3 } }\nprint(q === ____)", expect:"true", answer:"3", hint:"모양을 그대로 그려 안쪽까지", explain:"왼쪽 모양 <code>{ p: { q } }</code>이 오른쪽 구조와 맞물려 <b>안쪽 q</b>까지 꺼낸다 → 3."},
      {label:"스왑", ask:"임시변수 없이 두 값 교환하면 a는?", code:"let a = 1, b = 2\n;[a, b] = [b, a]\nprint(a === ____)", expect:"true", answer:"2", hint:"[a,b]=[b,a]", explain:"오른쪽 <code>[b, a]</code>=[2,1]을 만들어 <b>위치대로</b> a·b에 다시 담는다 → a=2, b=1. 임시변수 불필요."}
    ],
  }
  N["errors"] = {
    pattern: "🟡 보통 · 에러 처리 변형 — finally·message·중첩·흐름 (=== true 면 정답)",
    problems: [
      {label:"finally는 성공에도", ask:"에러가 없어도 finally 도나?", code:"let ran = false\ntry { let a = 1 } catch (e) { } finally { ran = true }\nprint(ran === ____)", expect:"true", answer:"true", hint:"finally=성공/실패 무관 항상", explain:"<code>finally</code>는 try가 <b>성공해도</b> 실행 → ran = true. '잡혔을 때만'이 아니라 '언제나'.", see:"errors"},
      {label:"message는 문자열", ask:"잡은 에러의 message 타입은 문자열?", code:"let has = false\ntry { null.x } catch (e) { has = (typeof e.message === \"string\") }\nprint(has === ____)", expect:"true", answer:"true", hint:"e.message = 사람이 읽는 설명(문자열)", explain:"에러 객체의 <code>message</code>는 <b>사람이 읽는 설명 문자열</b> → typeof는 \"string\" → true."},
      {label:"커스텀 메시지", ask:"throw new Error('품절')의 message는?", code:"let m = \"\"\ntry { throw new Error(\"품절\") } catch (e) { m = e.message }\nprint(m === \"____\")", expect:"true", answer:"품절", hint:"new Error(msg)의 msg가 e.message", explain:"<code>new Error(\"품절\")</code>로 던지면 <code>e.message</code>가 \"품절\" — 내 규칙 위반을 이렇게 알린다.", see:"errors"},
      {label:"catch 후 흐름", ask:"catch로 잡은 뒤 코드도 도나?", code:"let steps = 0\ntry { throw 1 } catch (e) { steps = steps + 1 }\nsteps = steps + 1\nprint(steps === ____)", expect:"true", answer:"2", hint:"catch 1번 + 그 뒤 1번", explain:"throw로 catch 진입(steps=1), catch가 흐름을 복구해 <b>try/catch 뒤 코드도 실행</b>(steps=2) → 2."},
