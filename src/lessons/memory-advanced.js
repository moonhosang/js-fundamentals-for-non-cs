// 🧠 메모리 심화 챕터 — 콜 스택 · 클로저 · GC (모두 구현 완료). 함수(5강)·객체(8강) 뒤.
// 함수(5강)·객체(8강) 뒤에 오는 챕터. 콜 스택은 함수 호출의 삶과 죽음을 시뮬레이션한다.
//
// 오해: 함수가 끝나면 지역변수는 무조건 사라진다 · box=null 하면 객체가 즉시 삭제된다
// 왜:   클로저가 붙잡은 스코프는 힙에 살아남는다(스택의 예외) · GC는 '아무도 안 가리킬 때'만 회수
// 대비: 보통 프레임 pop→지역변수 소멸 vs 클로저=캡처된 변수 잔존 · 참조 남으면 GC 안 함(누수)
...
;(function () {
  ...
  const SCENARIO_STACK = {
    ...
    steps: [
      ...
      { line: 1, stack: [{ name: 'main', slots: [{ name: 'total', value: '(대기)', bad: true }] }], heap: {}, returning: { value: '1100' },
        note: '<b>⑤ 반환 — 값이 통로로 나오고 프레임 pop</b> — <code>return</code>이 <b>1100을 프레임 밖 반환 통로로</b> 내보내고 addTax를 <b>즉시 끝낸다</b>(프레임 pop, 지역 price 사라짐). 1100은 <b>통로에서 호출한 자리로</b> 가는 중 — total은 <b>아직 대기</b>.',
  ...
  const SCENARIO_CLOSURE = {
    ...
    steps: [
      { line: 7, stack: [{ name: 'main', slots: [{ name: 'next', value: '(대기)', bad: true }] }], heap: {},
        note: '<b>①호출 — <code>const next = makeCounter()</code> 시작</b> — makeCounter를 부르려 한다. <b>아직 makeCounter 프레임은 없다</b>(main만) — next는 반환을 기다린다(대기). (매개변수 없어 인자 평가는 생략.)',
        engine: '호출부(전역)에서 makeCounter() 호출 준비 — 아직 새 프레임 없음. 다음 순간 push.' },
      { line: 0, stack: [{ name: 'main', slots: [{ name: 'next', value: '(대기)', bad: true }] }, { name: 'makeCounter', slots: [] }], heap: {},
        note: '<b>②프레임 push</b> — 그 호출로 makeCounter 실행 컨텍스트가 스택에 쌓인다(push). <b>매개변수 없음</b>, 그리고 <b>아직 count도 없다</b>(본문 실행 전 — 프레임만 등장, 슬롯 빈칸).',
      ...
      { line: 2, stack: [{ name: 'main', slots: [{ name: 'next', value: '(대기)', bad: true }] }], heap: { h1: { person: '🔒', name: '클로저', fields: [{ key: 'count', value: '0' }] } }, returning: { ref: 'h1', label: '클로저(안쪽 함수)' },
        note: '<b>⑤ 반환 — 안쪽 함수가 통로로 나오고 프레임 pop</b> — makeCounter가 <b>안쪽 함수(+ 붙잡은 count)</b>를 <b>반환 통로로</b> 내보내고(통로엔 h1을 가리키는 화살표) <b>장부는 pop(사라짐)</b>. 그런데 <b>count는 값 메모리에 살아남았다!</b> 반환값(클로저 h1)은 <b>호출한 자리로</b> 가는 중 — next는 <b>아직 대기</b>.',
      ...
      { line: 8, stack: [{ name: 'main', slots: [{ name: 'next', ref: 'h1' }] }, { name: '(익명)', slots: [] }], heap: { h1: { person: '🔒', name: '클로저', fields: [{ key: 'count', value: '1' }] } }, returning: { value: '1', discarded: true },
        note: 'next() 호출 → <b>(익명) 프레임이 push</b>돼 실행된다(호출=push, 끝나면 pop). 그 함수가 클로저의 <b>count를 +1 → 1</b>하고 <code>return count</code>로 <b>1을 돌려준다</b>(여기선 <code>next()</code>만 써서 안 담음). 밖에선 count를 직접 못 보고 next()로만 만진다.',
        engine: 'next() 호출 = 익명 함수 활성 레코드 push. 몸통이 힙 context의 count를 읽어 +1 → 1로 갱신(SMI 값 덮어씀). 프레임은 끝나면 pop되지만 힙 context는 그대로 남는다.' },
      { line: 9, stack: [{ name: 'main', slots: [{ name: 'next', ref: 'h1' }] }, { name: '(익명)', slots: [] }], heap: { h1: { person: '🔒', name: '클로저', fields: [{ key: 'count', value: '2' }] } }, returning: { value: '2', discarded: true },
        note: '또 next() → 다시 <b>프레임 push</b> → count <b>+1 → 2</b>하고 <b>2를 돌려준다</b>(역시 안 담음). count가 프레임과 함께 사라지지 않고 <b>기억</b>된다 — 이게 클로저의 힘(상태를 숨겨 보관).',
