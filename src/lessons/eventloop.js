  ...
  window.Lessons['eventloop'] = function render(root) {
    ...
    root.querySelector('[data-m="elv"]').append(EventLoopViz({
      ...
      steps: [
        ...
        { line: 3, phase: 'idle', stack: [], webapi: [], macro: ['() => print("배달 도착")'], out: ['주문 시작', '다음 손님 받기'], note: '<b>1초 경과 → 타이머 끝!</b> 브라우저가 콜백을 🌐 Web API에서 꺼내 <b>🟠 매크로 큐로 옮긴다</b>. 이제 "실행 대기" 상태.' },
        { line: 4, phase: 'drain-macro', stack: [{ label: '콜백', from: 'macro' }], webapi: [], macro: [], out: ['주문 시작', '다음 손님 받기', '배달 도착'], note: '콜스택이 비어 있으니 <b>이벤트 루프가 🟠 큐에서 콜백을 꺼내 콜스택에 올려 실행</b> → "배달 도착" 출력. <b>이 왕복 전체(위임 → 대기 → 큐 → 스택)가 이벤트 루프.</b>' },
        { line: 4, phase: 'idle', stack: [], webapi: [], macro: [], out: ['주문 시작', '다음 손님 받기', '배달 도착'], note: '전부 비었다 — 끝. <b>한 명(싱글 스레드)이지만, 느린 일을 위임하고 큐에서 꺼내 쓰니 안 멈추고 번갈아</b> 처리했다.' },
      ],
    }))
