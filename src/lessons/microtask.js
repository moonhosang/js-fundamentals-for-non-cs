  ...
  window.Lessons['microtask'] = function render(root) {
    ...
    root.querySelector('[data-m="elv"]').append(EventLoopViz({
      ...
      code: [
        ...
        'print("4")                                // 동기',
        '// 동기 끝 · 콜스택이 비었다 → 이벤트 루프가 큐에서 꺼내 실행',
        '//   (🟣마이크로 큐를 전부 실행한 뒤 → 🟠매크로 큐에서 하나 실행)',
      ],
      steps: [
        ...
        { line: 4, phase: 'idle', stack: [], micro: ['() => print("3")'], macro: ['() => print("2")'], out: ['1', '4'], note: '동기 코드가 다 끝나 <b>콜스택이 텅 비었다</b>(지금 실행 중인 코드 없음). 그러면 <b>이벤트 루프</b>가 "큐에 기다리는 콜백이 있나?" <b>확인한다</b>(="큐를 본다"). <b>규칙: 🟣마이크로 큐부터 전부 꺼내 실행</b> — 🟠매크로는 아직 차례가 아니다.' },
