...
;(function () {
  ...
  const SCENARIO_STACK_FAIL = {
    ...
    steps: [
      { line: 0, stack: [{ name: 'main', slots: [] }, { name: 'makeCard', slots: [] }], heap: {}, note: '<b>②프레임 push</b> — <code>makeCard()</code> 호출 → makeCard 칸이 스택에 쌓인다. <b>매개변수 없음</b> — 빈 칸. <b>아직 본문 실행 전.</b>', engine: '새 <b>스택 프레임(실행 컨텍스트)</b> 생성. <b>스코프·호이스팅</b>·<b>this·arguments</b> 준비. 매개변수가 없어 바인딩 단계도 없다. 배치는 스펙 비강제.' },
      { line: 1, stack: [{ name: 'main', slots: [] }, { name: 'makeCard', slots: [{ name: '(객체)', value: '{ name: "민지" }' }] }], heap: {}, note: '<b>④본문 실행</b> · <b>상상</b>: <code>return { name: "민지" }</code>에서 만든 객체를 힙이 아니라 <b>makeCard 칸(스택) 안</b>에 통째로 뒀다.', engine: '<b>가상의 시나리오</b> — 실제 V8은 객체를 이렇게 프레임 안에 통째로 두지 않고 <b>힙에 할당</b>하고 슬롯엔 포인터만 둔다. 이 그림은 "왜 힙이 필요한가"를 보이려는 반례.' },
