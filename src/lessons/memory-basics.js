...
//
// 오해: let y=x면 x·y가 같은 10 하나에 같이 붙는다 · money=200은 100이 200으로 "변신"한다
// 왜:   원시값은 대입 시 각자 셀로 복사(독립) · 불변이라 재할당=이름표를 새 값 셀로 옮기는 것
// 대비: 원시=값 복사(독립) vs 객체=주소 복사(공유) — 가변성이 복사/공유를 가른다
...
;(function () {
    ...
    root.querySelector('[data-m="letconst-viz"]').append(MemoryModel({
      ...
      steps: [
        ...
        { line: 1, stack: [{ name: 'main', slots: [{ name: 'score', ref: 'sc20' }] }], heap: { sc10: { label: '10', prim: true, faded: true }, sc20: { label: '20', prim: true, bad: true } },
          ...
          engine: '실제 V8: 20도 SMI라 score 슬롯 비트를 <b>10→20으로 제자리 덮어씀</b>(새 힙 셀 할당 없음). 만약 소수였다면 <b>HeapNumber</b> 새 박스+포인터 재연결 — 그땐 개념 그림(새 셀·화살표 이동)이 물리적으로도 맞다. <b>그래도 "불변"은 안 깨진다</b>: 불변은 <b>언어가 보장하는 의미</b>(네 코드로는 옛 값 10 자체를 못 고침)이지 엔진의 저장 방식이 아니다 — 슬롯 비트 덮어쓰기는 내부 최적화일 뿐, 관찰되는 동작은 개념 그림("새 값을 가리킴")과 똑같다.' },
  ...
  const SCENARIO_PUSHPOP = {
    ...
    steps: [
      { line: 7, stack: [{ name: 'main', slots: [{ name: 'say', value: '(대기)', bad: true }] }], heap: {},
        note: '<b>①인자 평가</b> — <code>const say = welcome("민지")</code>는 <b>본문으로 바로 안 뛴다</b>. 먼저 괄호 안 인자 <b>"민지"</b>를 값으로 준비한다. <b>아직 welcome 프레임은 없다</b> — 지금은 <b>main 프레임만</b>. say는 반환을 기다린다(대기).',
        engine: '인자는 <b>호출자(main) 쪽</b>에서 먼저 평가된다. welcome 프레임은 아직 없음 — 다음 순간 push. 문자열 "민지"는 힙 객체라 포인터로 전달될 값.' },
      { line: 3, stack: [{ name: 'main', slots: [{ name: 'say', value: '(대기)', bad: true }] }, { name: 'welcome', slots: [] }], heap: {},
        note: '<b>②프레임 push</b> — 그 호출로 스택에 <b>welcome 칸</b>이 새로 쌓인다. <b>매개변수 name 칸은 아직 비어 있다</b>(바인딩 전). (main의 say는 대기 — 반환을 기다린다)',
        engine: '새 <b>스택 프레임(실행 컨텍스트)</b> 생성. 이때 <b>스코프·호이스팅</b>(var·함수선언 등록, let/const는 TDZ)과 <b>this·arguments</b>도 준비된다 — 컴파일 때 만든 스코프 정보로. 배치는 스펙 비강제.' },
      { line: 3, stack: [{ name: 'main', slots: [{ name: 'say', value: '(대기)', bad: true }] }, { name: 'welcome', slots: [{ name: 'name', value: '"민지"' }] }], heap: {},
        note: '<b>③매개변수 바인딩</b> — 인수 "민지"를 welcome의 <b>name</b>에 복사(문자열은 힙 불변 객체라 실제론 <b>주소 복사</b>). 이제 name 칸이 찼다.',
        engine: '인자 값복사 — name 슬롯에 "민지"의 포인터를 복사. 원시(숫자)였다면 값 비트 복사였을 것.' },
      { line: 4, stack: [{ name: 'main', slots: [{ name: 'say', value: '(대기)', bad: true }] }, { name: 'welcome', slots: [{ name: 'name', value: '"민지"' }] }], heap: {},
        note: '<b>①인자 평가(안쪽 호출)</b> — welcome 본문 <code>let msg = makeGreeting(name)</code>도 <b>본문으로 바로 안 뛴다</b>. 먼저 인자 <b>name("민지")</b>을 값으로 준비한다. <b>아직 makeGreeting 프레임은 없다</b>(main·welcome <b>2개</b>) — welcome의 msg는 대기.',
        engine: '안쪽 호출도 같은 규약 — 인자(name)부터 평가된다. makeGreeting 프레임은 아직 없음 — 다음 순간 push.' },
      { line: 0, stack: [{ name: 'main', slots: [{ name: 'say', value: '(대기)', bad: true }] }, { name: 'welcome', slots: [{ name: 'name', value: '"민지"' }] }, { name: 'makeGreeting', slots: [] }], heap: {},
        note: '<b>②프레임 push</b> — 그 호출로 <b>또 push</b>. 이제 프레임 <b>3개</b>(main·welcome·makeGreeting), 맨 위 makeGreeting의 <b>name 칸은 아직 빈칸</b>(바인딩 전).',
        engine: '또 프레임 push — makeGreeting 실행 컨텍스트가 쌓인다. 매개변수 슬롯은 바인딩 전이라 비어 있다.' },
      { line: 0, stack: [{ name: 'main', slots: [{ name: 'say', value: '(대기)', bad: true }] }, { name: 'welcome', slots: [{ name: 'name', value: '"민지"' }] }, { name: 'makeGreeting', slots: [{ name: 'name', value: '"민지"' }] }], heap: {},
        note: '<b>③바인딩</b> — makeGreeting의 name에 인자를 복사. makeGreeting의 name은 <b>이 프레임만의 지역 슬롯</b> — welcome의 name과 이름만 같고 <b>별개 슬롯</b>이다.',
        engine: '각 프레임의 name은 독립된 지역 슬롯. 문자열이라 주소(포인터) 복사.' },
      { line: 1, stack: [{ name: 'main', slots: [{ name: 'say', value: '(대기)', bad: true }] }, { name: 'welcome', slots: [{ name: 'name', value: '"민지"' }] }, { name: 'makeGreeting', slots: [{ name: 'name', value: '"민지"' }] }], heap: {},
        note: '<b>④ makeGreeting 본문 — 반환값이 <u>지금</u> 만들어진다</b> — makeGreeting가 <b>아직 살아 있는 채</b> <code>"안녕, " + name + "님"</code>을 이어붙여 <b>반환값 "안녕, 민지님"을 만든다</b>. <b>아직 pop 전</b> — 값이 프레임 안에서 갖춰지는 순간.',
        engine: '문자열 이어붙이기로 <b>새 문자열</b>(힙)이 만들어진다. makeGreeting 프레임은 아직 살아 있고, 이 값의 포인터가 반환 준비된다. (다음 스텝에서 pop.)' },
      { line: 5, stack: [{ name: 'main', slots: [{ name: 'say', value: '(대기)', bad: true }] }, { name: 'welcome', slots: [{ name: 'name', value: '"민지"' }] }], heap: {}, returning: { value: '"안녕, 민지님"' },
        note: '<b>⑤ makeGreeting 반환·pop</b> — makeGreeting가 <b>만든 값을 프레임 밖 반환 통로로 내보내고</b> 그 칸은 <b>통째로 pop(사라짐)</b>. 반환값 "안녕, 민지님"은 <b>welcome의 msg 자리로</b> 가는 중 — msg는 <b>아직 대기</b>.',
        engine: '반환값은 <b>레지스터</b>로 넘어가고 makeGreeting 프레임은 <b>즉시 pop(회수)</b>. "안녕, 민지님"은 이어붙여 만든 <b>새 문자열</b>(힙)이라 프레임이 사라져도 살아남는다. 대입은 아직 — welcome이 받아야 한다.' },
      { line: 5, stack: [{ name: 'main', slots: [{ name: 'say', value: '(대기)', bad: true }] }, { name: 'welcome', slots: [{ name: 'name', value: '"민지"' }, { name: 'msg', value: '"안녕, 민지님"' }] }], heap: {},
        note: '<b>⑥ welcome이 대입</b> — welcome이 반환값을 받아 자기 지역 <b>msg</b>에 담는다. 이제 welcome은 name·msg 둘. (<b>안쪽 호출도 반환→대입 두 단계</b>다.)',
        engine: 'msg 슬롯에 힙 문자열 포인터 안착. makeGreeting 프레임은 이미 없다.' },
      { line: 5, stack: [{ name: 'main', slots: [{ name: 'say', value: '(대기)', bad: true }] }, { name: 'welcome', slots: [{ name: 'name', value: '"민지"' }, { name: 'msg', value: '"안녕, 민지님"' }] }], heap: {},
        note: '<b>④ welcome 본문 — 반환값이 <u>지금</u> 만들어진다</b> — welcome이 <b>아직 살아 있는 채</b> <code>"🎉 " + msg</code>로 <b>반환값 "🎉 안녕, 민지님"을 만든다</b>. <b>아직 pop 전</b>.',
        engine: '또 <b>새 문자열</b>(힙) 생성. welcome 프레임은 아직 살아 있고 반환값이 갖춰진다 — 다음 스텝에서 pop.' },
      { line: 7, stack: [{ name: 'main', slots: [{ name: 'say', value: '(대기)', bad: true }] }], heap: {}, returning: { value: '"🎉 안녕, 민지님"' },
        note: '<b>⑤ welcome 반환·pop</b> — welcome이 <b>만든 값을 밖 반환 통로로 내보내고</b> <b>pop</b>. name·msg는 <b>사라진다</b>. 반환값 "🎉 안녕, 민지님"은 <b>main의 say 자리로</b> 가는 중 — say는 <b>아직 대기</b>.',
        engine: 'welcome 프레임 pop = name·msg 슬롯 회수. 반환한 새 문자열은 힙에 남는다. 대입은 아직 — main이 받아야 한다.' },
      { line: 7, stack: [{ name: 'main', slots: [{ name: 'say', value: '"🎉 안녕, 민지님"' }] }], heap: {},
        note: '<b>⑥ 호출부 대입</b> — 호출한 자리 <code>const say = …</code>가 반환값을 받아 <b>say에 담는다</b>. <b>프레임은 사라져도 결과는 남는다</b>(return으로 빼냈으니까 — §4의 이유). 반환(⑤)과 대입(⑥)은 <b>별개의 두 단계</b>.',
        engine: '반환한 문자열은 힙에 남아 say가 가리킨다. welcome 프레임은 이미 pop된 뒤. 스택/힙 배치는 스펙 강제 아님.' },
    ],
  }
  ...
  const SCENARIO_SCOPE = {
    ...
    steps: [
      ...
      { line: 5, stack: [{ name: 'main · 전역 장부', slots: [{ name: 'appName', value: '"메모장"' }] }], heap: {},
        note: '<b>①인자 평가</b> — <code>greet("민지")</code>는 <b>본문으로 바로 안 뛴다</b>. 먼저 인자 "민지"를 값으로 읽는다(식이면 여기서 계산). <b>아직 greet 장부(프레임)는 없다.</b>',
        engine: '인자는 <b>호출자(전역) 쪽</b>에서 먼저 평가돼 전달 준비된다.' },
      { line: 1, stack: [{ name: 'main · 전역 장부', slots: [{ name: 'appName', value: '"메모장"' }] }, { name: 'greet · 함수 장부', slots: [] }], heap: {},
        note: '<b>②프레임 push</b> — <b>새 장부(greet 프레임)가 push</b>. 매개변수 <b>user 칸은 아직 비어 있다</b>(바인딩 전).',
        engine: '새 <b>스택 프레임(실행 컨텍스트)</b> 생성. <b>스코프·호이스팅</b>(var·함수선언 등록, let/const는 TDZ)·<b>this·arguments</b> 준비 — 컴파일 때 만든 스코프 정보로. 프레임은 스택 위로 <b>순서대로</b> 쌓인다. 배치는 스펙 비강제.' },
      { line: 1, stack: [{ name: 'main · 전역 장부', slots: [{ name: 'appName', value: '"메모장"' }] }, { name: 'greet · 함수 장부', slots: [{ name: 'user', value: '"민지"' }] }], heap: {},
        note: '<b>③매개변수 바인딩</b> — 인자 "민지"를 <b>user</b>에 복사(문자열은 <b>주소 복사</b>). user는 이 <b>greet 장부</b>의 지역변수.',
        engine: '인자 값복사(문자열은 포인터 복사)로 user 지역 슬롯을 채운다.' },
      ...
      { line: 3, stack: [{ name: 'main · 전역 장부', slots: [{ name: 'appName', value: '"메모장"' }] }, { name: 'greet · 함수 장부', slots: [{ name: 'user', value: '"민지"' }, { name: 'hi', value: '"메모장 · 민지 님"' }] }], heap: {}, returning: { value: '"메모장 · 민지 님"', discarded: true },
        note: '<b>⑤ 반환 — 값을 내보내지만 <u>아무도 안 받는다</u></b> — <code>return hi</code>가 hi 값 <b>"메모장 · 민지 님"</b>을 <b>프레임 밖 반환 통로로</b> 내보낸다. <b>return은 분명히 일어난다</b> — 그런데 호출부 <code>greet("민지")</code>가 <b>아무 변수에도 안 담아</b> 그 반환값은 <b>그냥 버려진다</b>. (담으려면 <code>let x = greet("민지")</code> — M2 §3·5강 5-4.)',
        engine: '반환값(문자열 포인터)이 레지스터로 나가지만 호출부가 받지 않아 곧 <b>도달 불가</b>(GC 대상)가 된다. 다음 순간 프레임 pop.' },
      { line: 5, stack: [{ name: 'main · 전역 장부', slots: [{ name: 'appName', value: '"메모장"' }] }], heap: {},
        note: '방금 return을 끝냈으니 greet 끝 → <b>함수 장부 pop</b>. user·hi(지역변수)는 <b>사라진다</b>. 바깥(전역)에선 hi를 <b>못 쓴다</b>(스코프 밖 = 접근 불가). appName(전역)은 그대로.',
  ...
  const SCENARIO_STACK_FAIL = {
    ...
    steps: [
      { line: 3, stack: [{ name: 'main', slots: [{ name: 'c', value: '(대기)', bad: true }] }], heap: {}, note: '<b>①호출 — <code>let c = makeCard()</code> 시작</b> — makeCard를 부르려 한다. <b>아직 makeCard 프레임도 없다</b>(main만) — c는 반환을 기다린다(대기). (매개변수 없어 인자 평가는 생략.)', engine: '호출부에서 makeCard() 호출 준비 — 아직 새 프레임 없음. 다음 순간 push.' },
      { line: 0, stack: [{ name: 'main', slots: [{ name: 'c', value: '(대기)', bad: true }] }, { name: 'makeCard', slots: [] }], heap: {}, note: '<b>②프레임 push</b> — 그 호출로 makeCard 칸이 스택에 쌓인다. <b>매개변수 없음</b> — 빈 칸. <b>아직 본문 실행 전.</b>', engine: '새 <b>스택 프레임(실행 컨텍스트)</b> 생성. <b>스코프·호이스팅</b>·<b>this·arguments</b> 준비. 매개변수가 없어 바인딩 단계도 없다. 배치는 스펙 비강제.' },
      { line: 1, stack: [{ name: 'main', slots: [{ name: 'c', value: '(대기)', bad: true }] }, { name: 'makeCard', slots: [{ name: '(객체)', value: '{ name: "민지" }' }] }], heap: {}, note: '<b>④본문 실행</b> · <b>상상</b>: <code>return { name: "민지" }</code>에서 만든 객체를 힙이 아니라 <b>makeCard 칸(스택) 안</b>에 통째로 뒀다.', engine: '<b>가상의 시나리오</b> — 실제 V8은 객체를 이렇게 프레임 안에 통째로 두지 않고 <b>힙에 할당</b>하고 슬롯엔 포인터만 둔다. 이 그림은 "왜 힙이 필요한가"를 보이려는 반례.' },
      { line: 1, stack: [{ name: 'main', slots: [{ name: 'c', value: '(대기)', bad: true }] }, { name: 'makeCard', slots: [{ name: '(객체)', value: '{ name: "민지" }' }] }], heap: {}, note: '<b>⑤ 반환 시도</b> — 이 객체를 <b>밖(반환 통로)으로</b> 내보내 c에 담으려 한다. <b>그런데</b> 이 객체는 힙이 아니라 <b>프레임 안</b>에 산다 — 프레임이 사라지면 같이 죽어서, <b>통로로 온전히 못 내보낸다</b>(값만 넘기는 통로에 큰 묶음의 실물은 못 실린다). 그래서 정상 반환 시뮬과 달리 <b>여긴 통로가 안 뜬다</b>.', engine: '만약 프레임 안 실물을 반환한다면 그 실체는 프레임에 묶여 있다 — 반환은 보통 레지스터로 값(포인터)만 넘기는데, 프레임에 사는 큰 묶음은 그렇게 살려낼 수 없다. 힙 객체라면 포인터가 통로로 나가 살아남는다(그게 힙을 쓰는 이유).' },
      { line: 3, stack: [{ name: 'main', slots: [{ name: 'c', ref: 'dead' }] }], heap: { dead: { freed: true } }, noReturnLane: true, note: 'makeCard가 끝나 칸이 <b>pop(치워짐)</b> → M2에서 봤듯 <b>그 안의 객체도 함께 사라진다</b>. c는 없어진 메모리를 가리킨다 💥. <b>그래서</b> 오래 살아야 할 묶음은 스택이 아니라 <b>힙</b>에 둔다.', engine: '프레임 pop = 그 슬롯 메모리 회수 → 실물이 프레임에 있었다면 <b>dangling(허상) 포인터</b>가 된다. V8이 <b>힙 할당 + GC</b>를 쓰는 이유가 바로 이것 — 힙 객체는 프레임이 사라져도 도달 가능하면 산다.' },
    ],
  }
  ...
  const SCENARIO_PASSVAL = {
    ...
    steps: [
      ...
      { line: 4, stack: [{ name: 'main', slots: [{ name: 'money', value: '10000' }] }], heap: {}, note: '<b>①인자 평가</b> — <code>tear(money)</code>는 <b>본문으로 바로 안 뛴다</b>. 먼저 괄호 안 <code>money</code>를 값으로 읽어 <b>10000</b>을 준비한다(식이면 여기서 계산). <b>아직 tear 프레임은 없다.</b>', engine: '인자는 <b>호출자(main) 쪽</b>에서 먼저 평가돼, 보통 <b>레지스터·스택</b>으로 전달 준비된다.' },
      { line: 1, stack: [{ name: 'main', slots: [{ name: 'money', value: '10000' }] }, { name: 'tear', slots: [] }], heap: {}, note: '<b>②프레임 push</b> — tear의 <b>실행 컨텍스트(장부 칸)</b>가 스택에 새로 쌓인다. <b>매개변수 bill 칸은 아직 비어 있다</b>(바인딩 전).', engine: '새 <b>스택 프레임(실행 컨텍스트)</b> 생성. 이때 <b>스코프·호이스팅</b>(var·함수선언 등록, let/const는 TDZ)과 <b>this·arguments</b>도 준비된다 — 컴파일 때 만든 스코프 정보로. 배치는 스펙 비강제.' },
      { line: 1, stack: [{ name: 'main', slots: [{ name: 'money', value: '10000' }] }, { name: 'tear', slots: [{ name: 'bill', ref: 'bill10000' }] }], heap: { bill10000: { label: '10000', prim: true } }, note: '<b>③매개변수 바인딩(복사)</b> — money의 값 <b>10000을 복사</b>해 bill에 담는다. bill은 money와 <b>별개의 슬롯</b>(복사된 만원 한 장 더). ← <b>이게 "복사본"의 순간.</b>', engine: '인자 <b>값복사</b> — bill 슬롯에 10000 비트 복사(각자 인라인 SMI, 별개 슬롯). 객체였다면 여기서 <b>주소(포인터)</b>가 복사됐을 것(그래서 공유).' },
      { line: 2, stack: [{ name: 'main', slots: [{ name: 'money', value: '10000' }] }, { name: 'tear', slots: [{ name: 'bill', ref: 'bill0' }] }], heap: { bill10000: { label: '10000', prim: true, faded: true }, bill0: { label: '0', prim: true, bad: true } }, note: '<b>④이제야 본문 첫 줄</b> <code>bill = 0</code> — <b>숫자는 불변</b>. 10000이 0으로 변신한 게 아니라 bill의 <b>화살표가 새 0 셀로 이동</b>, 옛 10000은 버려진다(흐림). <b>money(원본)는 그대로 10000</b> — 애초에 별개 슬롯.', engine: '실제 V8: 0도 SMI라 bill 슬롯 비트를 <b>10000→0으로 제자리 덮어씀</b>. money 슬롯은 손도 안 댐.' },
      { line: 4, stack: [{ name: 'main', slots: [{ name: 'money', value: '10000' }] }], heap: {}, returning: { value: 'undefined', discarded: true }, note: '<b>⑤ 반환·pop</b> — tear는 <b>명시적 <code>return</code>이 없어도</b> 함수라서 <b><code>undefined</code>를 통로로 내보낸다</b>(<code>tear(money)</code>가 안 받아 💨 버려짐). 그리고 프레임 <b>pop</b>, bill(복사본) 사라짐. money는 여전히 <b>10000</b> — 원시값은 <b>복사본</b>으로 전달돼 원본이 안전하다.', engine: 'tear 프레임 pop = bill 슬롯 회수. 인라인 원시값이라 힙에 남을 것도 GC 대상도 없다. return이 없으면 엔진이 <b>싱글턴 undefined</b>를 반환값으로 넘긴다.' },
    ],
  }
  ...
  const SCENARIO_XY_COPY = {
    ...
    steps: [
      { line: 0,
        stack: [{ name: 'main', slots: [{ name: 'x', ref: 'x10' }] }],
        heap: { x10: { label: '10', prim: true } },
        note: 'x는 값 메모리의 <b>10</b>을 가리킨다(장부엔 이름+화살표, 값은 값 메모리에).',
        engine: '<b>10</b>은 <b>SMI(작은 정수)</b> — V8은 힙에 안 두고 <b>x 슬롯 안에 태그 비트로 인라인</b>. 실제론 여기 \'셀\'도 \'화살표\'도 없다(그건 개념 그림). 이 태깅은 <b>V8 방식</b>이고 스펙은 저장 방식을 강제하지 않는다.' },
      { line: 1,
        stack: [{ name: 'main', slots: [{ name: 'x', ref: 'x10' }, { name: 'y', ref: 'y10' }] }],
        heap: { x10: { label: '10', prim: true }, y10: { label: '10', prim: true } },
        note: '😵 착각: "x·y가 <b>같은 10 하나</b>를 같이 가리킨다"? ❌ 원시값은 <b>값 복사</b> → y는 <b>자기 10</b>(별개 셀). <b>셀이 둘</b>이다.',
        engine: 'y 슬롯에 x의 10 <b>비트를 복사</b>(각자 인라인 SMI · 별개 슬롯). \'두 셀\'은 개념 그림 — 물리적으론 <b>두 변수 슬롯 각각에 10</b>. (만약 <b>문자열</b>이었다면 리터럴은 <b>인터닝</b>돼 실제로 한 힙 문자열을 공유했을 수도 — 타입마다 다르다.)' },
      { line: 2,
        stack: [{ name: 'main', slots: [{ name: 'x', ref: 'x10' }, { name: 'y', ref: 'y20' }] }],
        heap: { x10: { label: '10', prim: true }, y10: { label: '10', prim: true, faded: true }, y20: { label: '20', prim: true, bad: true } },
        note: '<b>숫자는 불변</b> — 10이 20으로 변신하는 게 아니다. y의 화살표가 <b>새 20 셀로 이동</b>하고, y가 쓰던 <b>옛 10은 아무도 안 가리켜 버려진다</b>(흐림). <b>x의 10은 그대로</b> — 별개 셀이라 안 움직인다.',
        engine: '실제 V8: 20도 SMI라 <b>y 슬롯 비트를 10→20으로 제자리 덮어씀</b>(새 힙 셀 할당 없음). 만약 <b>소수(20.5)</b>였다면 <b>HeapNumber(불변 힙 박스)</b>를 새로 만들고 y가 그 <b>포인터</b>로 재연결 — 그땐 개념 그림(새 셀+화살표 이동)이 물리적으로도 맞다. <code>true·undefined</code>는 <b>싱글턴 하나를 공유</b>. 무엇도 스펙 강제가 아니며 JSC·SpiderMonkey는 <b>NaN-boxing</b>으로 또 다르다.' },
    ],
  }
  ...
  const SCENARIO_PASSOBJ = {
    ...
    steps: [
      ...
      { line: 4, stack: [{ name: 'main', slots: [{ name: 'hero', ref: 'h1' }] }], heap: { h1: { fields: [{ key: 'name', value: '"용사"' }, { key: 'level', value: '1' }] } }, note: '<b>①인자 평가</b> — <code>levelUp(hero)</code>는 본문으로 바로 안 뛴다. 먼저 <code>hero</code>를 값으로 읽는다 — 그 값은 <b>주소 h1</b>(객체 실체가 아니라). <b>아직 levelUp 프레임은 없다.</b>', engine: '인자 hero는 호출자(main) 쪽에서 먼저 평가 — 슬롯에 담긴 <b>포인터(h1)</b>가 전달 준비된다.' },
      { line: 1, stack: [{ name: 'main', slots: [{ name: 'hero', ref: 'h1' }] }, { name: 'levelUp', slots: [] }], heap: { h1: { fields: [{ key: 'name', value: '"용사"' }, { key: 'level', value: '1' }] } }, note: '<b>②프레임 push</b> — levelUp 실행 컨텍스트가 스택에 쌓인다. <b>매개변수 user 칸은 아직 비어 있다</b>(바인딩 전).', engine: '새 <b>스택 프레임(실행 컨텍스트)</b> 생성. <b>스코프·호이스팅</b>·<b>this·arguments</b> 준비 — 컴파일 때 만든 스코프 정보로. 배치는 스펙 비강제.' },
      { line: 1, stack: [{ name: 'main', slots: [{ name: 'hero', ref: 'h1' }] }, { name: 'levelUp', slots: [{ name: 'user', ref: 'h1' }] }], heap: { h1: { fields: [{ key: 'name', value: '"용사"' }, { key: 'level', value: '1' }] } }, note: '<b>③매개변수 바인딩</b> — user에 hero의 <b>주소(h1)를 복사</b>. 객체는 <b>주소 복사=공유</b> — user와 hero가 <b>같은 h1 객체</b>(별칭)! (복사본 아님. 그래서 함수 안 변경이 원본에 보인다)', engine: '인자 값복사지만 넘어온 값이 <b>객체 주소(포인터)</b>라 user 슬롯에 그 주소가 복사된다(객체 자체는 복사 안 됨).' },
      ...
      { line: 4, stack: [{ name: 'main', slots: [{ name: 'hero', ref: 'h1' }] }], heap: { h1: { fields: [{ key: 'name', value: '"용사"' }, { key: 'level', value: '2' }] } }, returning: { value: 'undefined', discarded: true }, note: '<b>⑤ 반환·pop</b> — levelUp도 <b>명시적 <code>return</code>이 없어</b> <b><code>undefined</code>를 통로로 내보내고</b>(안 받아 💨 버려짐) 프레임 pop. 그래도 <b>hero.level은 2로 바뀐 채</b> — 객체는 주소로 전달돼 원본이 바뀐다. (M5의 원시값과 정반대!)', engine: 'levelUp 프레임 pop = user 슬롯 회수. 힙 객체는 hero가 여전히 가리켜 <b>도달 가능 → 생존</b>(GC 안 함). 변경은 이미 힙에 남았다. return이 없어 반환값은 undefined.' },
    ],
  }
...
let strong = levelUpSafe(hero)       // hero는 그대로, strong만 레벨업</pre>
  ...
  const SCENARIO_PASSARR = {
    ...
    steps: [
      ...
      { line: 4, stack: [{ name: 'main', slots: [{ name: 'cart', ref: 'h1' }] }], heap: { h1: { label: '["우유"]' } }, note: '<b>①인자 평가</b> — <code>addItem(cart, "빵")</code>은 본문으로 바로 안 뛴다. 먼저 인자 <b>둘</b>을 값으로 읽는다 — <code>cart</code>의 값은 <b>주소 h1</b>, <code>"빵"</code>은 문자열. <b>아직 addItem 프레임은 없다.</b>', engine: '인자는 호출자(main) 쪽에서 먼저 평가 — cart의 포인터(h1)와 "빵"이 전달 준비된다.' },
      { line: 1, stack: [{ name: 'main', slots: [{ name: 'cart', ref: 'h1' }] }, { name: 'addItem', slots: [] }], heap: { h1: { label: '["우유"]' } }, note: '<b>②프레임 push</b> — addItem 실행 컨텍스트가 스택에 쌓인다. <b>매개변수 list·item 칸은 아직 비어 있다</b>(바인딩 전).', engine: '새 <b>스택 프레임(실행 컨텍스트)</b> 생성. <b>스코프·호이스팅</b>·<b>this·arguments</b> 준비 — 컴파일 때 만든 스코프 정보로. 배치는 스펙 비강제.' },
      { line: 1, stack: [{ name: 'main', slots: [{ name: 'cart', ref: 'h1' }] }, { name: 'addItem', slots: [{ name: 'list', ref: 'h1' }, { name: 'item', value: '"빵"' }] }], heap: { h1: { label: '["우유"]' } }, note: '<b>③매개변수 바인딩</b> — list엔 cart의 <b>주소(h1)를 복사=공유</b>(list와 cart는 <b>같은 배열</b>!), item엔 "빵"을 복사. 그래서 함수 안 <code>push</code>가 원본에 보인다.', engine: 'list엔 배열 <b>주소(포인터) 복사</b>, item("빵")은 문자열 포인터 복사. 배열 실체는 하나.' },
      ...
      { line: 4, stack: [{ name: 'main', slots: [{ name: 'cart', ref: 'h1' }] }], heap: { h1: { label: '["우유", "빵"]' } }, returning: { value: 'undefined', discarded: true }, note: '<b>⑤ 반환·pop</b> — addItem도 <b>명시적 <code>return</code>이 없어</b> <b><code>undefined</code>를 통로로 내보내고</b>(안 받아 💨 버려짐) 프레임 pop. cart는 이제 <b>2칸</b> — 배열도 객체라 주소로 전달 = 원본 바뀜.', engine: 'addItem 프레임 pop = list·item 슬롯 회수. 배열은 cart가 가리켜 생존, 변경은 힙에 남는다. return이 없어 반환값은 undefined.' },
    ],
  }
