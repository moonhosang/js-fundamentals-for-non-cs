...
;(function () {
  ...
  const SCENARIO_PUSHPOP = {
    ...
    steps: [
      ...
      { line: 5, stack: [{ name: 'main', slots: [{ name: 'say', value: '(대기)', bad: true }] }, { name: 'welcome', slots: [{ name: 'name', value: '"민지"' }] }], heap: {},
        note: '<b>⑤ makeGreeting 반환·pop</b> — makeGreeting가 <b>만든 값을 프레임 밖으로 내보내고</b> 그 칸은 <b>통째로 pop(사라짐)</b>. 반환값 "안녕, 민지님"은 <b>welcome의 msg 자리로</b> 가는 중 — msg는 <b>아직 대기</b>.',
      ...
      { line: 7, stack: [{ name: 'main', slots: [{ name: 'say', value: '(대기)', bad: true }] }], heap: {},
        note: '<b>⑤ welcome 반환·pop</b> — welcome이 <b>만든 값을 밖으로 내보내고</b> <b>pop</b>. name·msg는 <b>사라진다</b>. 반환값 "🎉 안녕, 민지님"은 <b>main의 say 자리로</b> 가는 중 — say는 <b>아직 대기</b>.',
  ...
  const SCENARIO_SCOPE = {
    ...
    steps: [
      ...
      { line: 3, stack: [{ name: 'main · 전역 장부', slots: [{ name: 'appName', value: '"메모장"' }] }, { name: 'greet · 함수 장부', slots: [{ name: 'user', value: '"민지"' }, { name: 'hi', value: '"메모장 · 민지 님"' }] }], heap: {},
        note: '<b>⑤ 반환 — 값을 내보내지만 <u>아무도 안 받는다</u></b> — <code>return hi</code>가 hi 값 <b>"메모장 · 민지 님"</b>을 <b>프레임 밖으로</b> 내보낸다. <b>return은 분명히 일어난다</b> — 그런데 호출부 <code>greet("민지")</code>가 <b>아무 변수에도 안 담아</b> 그 반환값은 <b>그냥 버려진다</b>. (담으려면 <code>let x = greet("민지")</code> — M2 §3·5강 5-4.)',
  ...
  const SCENARIO_STACK_FAIL = {
    ...
    steps: [
      ...
      { line: 1, stack: [{ name: 'main', slots: [{ name: 'c', value: '(대기)', bad: true }] }, { name: 'makeCard', slots: [{ name: '(객체)', value: '{ name: "민지" }' }] }], heap: {}, note: '<b>⑤ 반환 시도</b> — 이 객체를 <b>밖으로 반환</b>해 c에 담으려 한다.', engine: '만약 프레임 안 실물을 반환한다면 그 실체는 프레임에 묶여 있다 — 반환은 보통 레지스터로 값만 넘기는데, 프레임에 사는 큰 묶음은 그렇게 살려낼 수 없다.' },
