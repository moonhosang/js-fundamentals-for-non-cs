  ...
  window.Lessons['class'] = function render(root) {
    ...
    root.querySelector('[data-m="mem"]').append(MemoryModel({
      ...
      steps: [
        ...
        { line: 0, stack: [{ name: 'main', slots: [{ name: 'a', value: '(대기)', bad: true }] }], heap: { h1: { label: 'Cat', fields: [{ key: 'hp', value: '100' }] } }, note: '<b>⑤ 반환·pop</b> — 생성자가 <b>this(h1)를 암묵 반환</b>하고 Cat 프레임은 <b>pop(사라짐)</b>. 반환된 h1 <b>주소</b>는 호출한 자리로 가는 중 — a는 <b>아직 대기</b>. <b>객체 h1은 힙에 살아남는다</b>(프레임과 무관).', engine: '생성자는 명시적 return이 없으면 this를 반환. Cat 프레임 pop → this 슬롯 소멸, 그러나 h1 객체는 힙에 남아 반환된다.' },
