...
;(function () {
  ...
  window.Lessons['5-1'] = function render(root) {
    ...
    root.querySelector('[data-m="mem"]').append(MemoryModel({
      ...
      steps: [
        { line: 3, stack: [{ name: 'main', slots: [{ name: 'a', value: '(대기)', bad: true }] }], heap: {}, note: '<code>withTax(10000)</code> 호출 직전 — a는 반환을 기다린다(대기).', engine: '<b>호출 직전</b> — 아직 <b>withTax 프레임은 없다</b>(스택엔 main뿐). a 슬롯은 값이 안 정해졌고 대기는 교육용 표식일 뿐. 인수 10000은 작은 정수라 <b>SMI</b>로 슬롯에 인라인될 값.' },
        { line: 1, stack: [{ name: 'main', slots: [{ name: 'a', value: '(대기)', bad: true }] }, { name: 'withTax', slots: [{ name: 'price', value: '10000' }] }], heap: {}, note: 'withTax 프레임 push — 인수 10000이 매개변수 price에 담긴다. <code>return price+price*0.1</code>이 <b>11000</b>을 만든다.', engine: '<b>withTax(10000) 호출</b> → 새 <b>스택 프레임(실행 컨텍스트)</b>이 push되고 인수 10000이 매개변수 price에 <b>값 복사</b>(작은 정수라 <b>SMI</b>로 슬롯 인라인, 힙 안 씀). 0.1은 소수라 <b>HeapNumber</b>(힙 박스+포인터). 이 배치는 V8 구현이고 스펙은 강제하지 않는다.' },
        { line: 3, stack: [{ name: 'main', slots: [{ name: 'a', value: '11000' }] }], heap: {}, note: '<b>11000</b>을 돌려주고 withTax 프레임은 <b>pop</b>. a에 담긴다. 부를 때마다 이렇게 프레임이 <b>잠깐</b> 생겼다 사라진다.', engine: '<b>return</b> → 결과 11000은 보통 <b>레지스터</b>로 넘어가 a 슬롯에 담기고, withTax 프레임은 <b>pop(즉시 회수)</b>. 지역 슬롯 price는 사라진다.' },
      ],
    }))
  ...
  window.Lessons['5-2'] = function render(root) {
    ...
    root.querySelector('[data-m="mem"]').append(MemoryModel({
      ...
      steps: [
        ...
        { line: 3, stack: [{ name: 'main', slots: [] }, { name: 'greet', slots: [] }], heap: {}, note: '<code>greet()</code> <b>호출</b> → 비로소 <b>greet 프레임이 push</b>된다. ( )가 그 신호.', engine: '<b>greet() 호출</b> → 새 <b>스택 프레임(실행 컨텍스트)</b>이 push된다. 매개변수가 없어 인자 값복사도, 지역 슬롯도 없다.' },
        { line: 3, stack: [{ name: 'main', slots: [] }], heap: {}, note: '<code>return "안녕!"</code> → 값을 돌려주고 greet 프레임은 <b>pop(사라짐)</b>. print가 "안녕!"을 찍는다.', engine: '<b>return "안녕!"</b> → 문자열은 힙에 <b>불변 객체</b>로 있고 그 <b>포인터</b>가 레지스터로 넘어간다(리터럴이라 인터닝될 수 있음). greet 프레임은 <b>pop(즉시 회수)</b>.' },
      ],
    }))
    wireGoto(root)
  }
  ...
  window.Lessons['5-3'] = function render(root) {
    ...
    root.querySelector('[data-m="mem"]').append(MemoryModel({
      ...
      steps: [
        { line: 3, stack: [{ name: 'main', slots: [] }], heap: {}, note: '<code>greet("민지")</code>를 호출하려 한다.', engine: '<b>호출 직전</b> — 아직 greet 프레임 없음. 인수 "민지"는 힙의 <b>문자열 객체</b>이고, 넘어가는 건 그 <b>포인터</b>다.' },
        { line: 1, stack: [{ name: 'main', slots: [] }, { name: 'greet', slots: [{ name: 'name', value: '"민지"' }] }], heap: {}, note: 'greet 프레임 push — <b>인수 "민지"가 매개변수 name에 담긴다</b>(name 셀 = "민지"). 이름은 장부, 값은 값 메모리.', engine: '<b>greet("민지") 호출</b> → 새 <b>스택 프레임</b> push. 문자열 인수는 <b>포인터 값복사</b>로 매개변수 name 슬롯에 담긴다(본체는 힙에 하나, 복사되는 건 주소). 이 배치는 V8 구현이며 스펙 비강제.' },
        { line: 3, stack: [{ name: 'main', slots: [] }], heap: {}, note: '<b>"안녕, 민지 님!"</b>을 돌려주고 greet 프레임 pop. name도 함께 사라진다.', engine: '<b>return</b> → 이어붙인 새 문자열의 <b>포인터</b>가 레지스터로 넘어가고 greet 프레임은 <b>pop</b>. 지역 슬롯 name은 사라진다(가리키던 "민지" 본체는 아무도 안 쓰면 추후 GC).' },
      ],
    }))
    wireGoto(root)
  }
  ...
  window.Lessons['5-4'] = function render(root) {
    ...
    root.querySelector('[data-m="mem"]').append(MemoryModel({
      ...
      steps: [
        { line: 3, stack: [{ name: 'main', slots: [{ name: 'r', value: '(대기)', bad: true }] }], heap: {}, note: '<code>double(5)</code> 호출 직전 — r은 반환을 <b>기다린다</b>(대기).', engine: '<b>호출 직전</b> — double 프레임은 아직 없음. r 슬롯은 값 미정(대기는 교육용). 인수 5는 작은 정수라 <b>SMI</b>.' },
        { line: 1, stack: [{ name: 'main', slots: [{ name: 'r', value: '(대기)', bad: true }] }, { name: 'double', slots: [{ name: 'x', value: '5' }] }], heap: {}, note: 'double 프레임: x=5. <code>return x*2</code>가 <b>10</b>을 만든다.', engine: '<b>double(5) 호출</b> → 새 <b>스택 프레임</b> push, 5가 매개변수 x에 <b>값 복사</b>(SMI 인라인). x*2도 정수라 SMI 범위.' },
        { line: 3, stack: [{ name: 'main', slots: [{ name: 'r', value: '10' }] }], heap: {}, note: '<b>return 10</b>을 돌려줘 <b>r 셀에 담긴다</b> ✔. (return이 없었다면 r엔 <b>undefined</b>가 담겼을 것.)', engine: '<b>return 10</b> → 값은 보통 <b>레지스터</b>로 넘어가 r 슬롯에 담기고 double 프레임은 <b>pop(즉시 회수)</b>. 지역 x는 사라진다. (return이 없었다면 싱글턴 <b>undefined</b>가 넘어갔을 것.)' },
      ],
    }))
    wireGoto(root)
  }
  ...
  window.Lessons['5-5'] = function render(root) {
    ...
    root.querySelector('[data-m="f1"]').append(MemoryModel({
      ...
      steps: [
        { line: 3, stack: [{ name: 'main', slots: [{ name: 'sum', value: '(대기)', bad: true }] }], heap: {}, note: '<code>add(3,4)</code> 호출 직전 — sum은 반환을 기다린다(대기).', engine: '<b>호출 직전</b> — add 프레임 없음. 인수 3·4는 작은 정수라 <b>SMI</b>로 넘어갈 값.' },
        { line: 1, stack: [{ name: 'main', slots: [{ name: 'sum', value: '(대기)', bad: true }] }, { name: 'add', slots: [{ name: 'a', value: '3' }, { name: 'b', value: '4' }] }], heap: {}, note: 'add 프레임 push. a=3, b=4. <code>return a+b</code>가 <b>7</b>을 만든다.', engine: '<b>add(3,4) 호출</b> → 새 <b>스택 프레임</b> push, 3·4가 각각 a·b 슬롯에 <b>값 복사</b>(SMI 인라인, 힙 안 씀). 중첩·재귀라면 이렇게 프레임이 여러 개 쌓인다.' },
        { line: 3, stack: [{ name: 'main', slots: [{ name: 'sum', value: '7' }] }], heap: {}, note: '<b>return 7</b>이 프레임 <b>밖으로</b> 나와 sum에 담기고, add 프레임은 <b>pop</b>. ✔ 값이 있다.', engine: '<b>return 7</b> → 값은 <b>레지스터</b>로 넘어가 sum에 담기고 add 프레임은 <b>pop(즉시 회수)</b>. 지역 a·b는 사라진다.' },
      ],
    }))
    root.querySelector('[data-m="f2"]').append(MemoryModel({
      ...
      steps: [
        { line: 3, stack: [{ name: 'main', slots: [{ name: 'r', value: '(대기)', bad: true }] }], heap: {}, note: '<code>shout(5)</code> 호출 직전.', engine: '<b>호출 직전</b> — shout 프레임 없음. 인수 5는 <b>SMI</b>.' },
        { line: 1, stack: [{ name: 'main', slots: [{ name: 'r', value: '(대기)', bad: true }] }, { name: 'shout', slots: [{ name: 'x', value: '5' }, { name: 'big', value: '500' }] }], heap: {}, note: 'shout 프레임: x=5, big=500 <b>계산만</b>. 그런데 <b>return이 없다</b>.', engine: '<b>shout(5) 호출</b> → 새 <b>스택 프레임</b> push. x=5, 지역 big=500 모두 이 프레임 슬롯에(정수라 SMI). 다만 <b>return이 없다</b>.' },
        { line: 3, stack: [{ name: 'main', slots: [{ name: 'r', value: 'undefined' }] }], heap: {}, note: '끝까지 return이 없으면 함수는 <b>자동으로 undefined</b>를 돌려준다(에러가 아니라 <b>정상적인 "값 없음"</b>). big(지역)은 사라지고 <b>r엔 undefined</b>.', engine: '끝까지 return이 없으면 엔진이 <b>싱글턴 undefined</b>를 반환값으로 넘긴다(에러 아님). shout 프레임은 <b>pop</b> → 지역 x·big 슬롯은 즉시 사라진다.' },
      ],
    }))
    ...
    root.querySelector('[data-m="f3"]').append(MemoryModel({
      ...
      steps: [
        { line: 4, stack: [{ name: 'main', slots: [] }], heap: {}, note: '<code>safeDiv(10, 0)</code> 호출.', engine: '<b>호출 직전</b> — safeDiv 프레임 없음. 인수 10·0은 작은 정수라 <b>SMI</b>.' },
        { line: 1, stack: [{ name: 'main', slots: [] }, { name: 'safeDiv', slots: [{ name: 'a', value: '10' }, { name: 'b', value: '0' }] }], heap: {}, note: 'safeDiv 프레임: a=10, b=0. <code>b===0</code>이 <b>참</b> → 여기서 <b>return "못 나눔"</b>을 만난다.', engine: '<b>safeDiv(10,0) 호출</b> → 새 <b>스택 프레임</b> push, a=10·b=0(SMI). if 분기는 같은 프레임 안 <b>제어 흐름</b>일 뿐 새 프레임을 만들지 않는다.' },
        { line: 1, stack: [{ name: 'main', slots: [] }], heap: {}, note: 'return을 만나는 <b>순간 함수는 즉시 끝</b> — <b>아래 <code>return a/b</code>는 실행조차 안 됨</b>(dead code). 프레임 pop, "못 나눔" 반환.', engine: '<b>조기 return</b> → "못 나눔"(힙 문자열)의 포인터가 레지스터로 넘어가고 프레임은 <b>즉시 pop</b>. 아래 <code>return a/b</code>는 실행 자체가 안 되니 어떤 슬롯·연산도 만들지 않는다.' },
      ],
    }))
    ...
    root.querySelector('[data-m="f4"]').append(MemoryModel({
      ...
      steps: [
        { line: 5, stack: [{ name: 'main', slots: [] }], heap: {}, note: '<code>grade(85)</code> 호출.', engine: '<b>호출 직전</b> — grade 프레임 없음. 인수 85는 <b>SMI</b>.' },
        { line: 1, stack: [{ name: 'main', slots: [] }, { name: 'grade', slots: [{ name: 'score', value: '85' }] }], heap: {}, note: 'score=85. 첫 <code>if(≥90)</code> <b>거짓</b> → 건너뛴다(이 return은 안 됨).', engine: '<b>grade(85) 호출</b> → 새 <b>스택 프레임</b> push, score=85(SMI). if 분기는 같은 프레임 안 제어 흐름이라 프레임 수는 그대로.' },
        { line: 2, stack: [{ name: 'main', slots: [] }, { name: 'grade', slots: [{ name: 'score', value: '85' }] }], heap: {}, note: '둘째 <code>if(≥80)</code> <b>참</b> → <b>return "B"</b>. 여기서 끝(셋째 줄 "C"는 안 옴).', engine: '둘째 if가 참이 돼 <b>return "B"</b>로 간다 — 아직 같은 grade 프레임 안. 돌려줄 "B"는 힙 문자열(포인터로 전달될 값).' },
        { line: 2, stack: [{ name: 'main', slots: [] }], heap: {}, note: '"B" 반환하고 pop. <b>점수가 달랐다면 다른 return</b>이 실행됐을 것 — 그래서 반환값은 "있을 수도, <b>다를 수도</b>".', engine: '<b>return "B"</b> → 문자열 포인터가 <b>레지스터</b>로 넘어가고 grade 프레임은 <b>pop(즉시 회수)</b>. 지역 score는 사라진다.' },
      ],
    }))
  ...
  window.Lessons['5-6'] = function render(root) {
    ...
    root.querySelector('[data-m="mem"]').append(MemoryModel({
      ...
      steps: [
        ...
        { line: 2, stack: [{ name: 'main', slots: [{ name: 'appName', value: '"메모장"' }] }, { name: 'makeMsg', slots: [{ name: 'user', value: '"민지"' }, { name: 'msg', value: '"민지 님"' }] }], heap: {}, note: 'makeMsg 프레임: <b>user(인수)·msg(지역변수)는 이 프레임 안에만</b> 있다.', engine: '<b>makeMsg("민지") 호출</b> → 새 <b>스택 프레임</b> push. user(인수 포인터 복사)·msg(지역)는 <b>이 프레임 슬롯에만</b> 산다. appName은 상위 프레임 슬롯이라 참조로 읽는다.' },
        { line: 5, stack: [{ name: 'main', slots: [{ name: 'appName', value: '"메모장"' }] }], heap: {}, note: 'makeMsg <b>pop</b> → user·msg <b>사라짐</b>(지역이라). appName(전역)은 <b>그대로</b>. 그래서 밖에서 msg는 <b>없다</b>.', engine: '<b>return</b> 뒤 makeMsg 프레임 <b>pop(즉시 회수)</b> → user·msg 슬롯은 사라진다. appName 슬롯은 main에 그대로. (지역이 함수 밖에도 살아남는 경우는 <b>클로저</b>로 힙 context에 승격될 때뿐 — 여긴 아님.)' },
      ],
    }))
    wireGoto(root)
  }
  ...
  window.Lessons['5-7'] = function render(root) {
    ...
    root.querySelector('[data-m="mem"]').append(MemoryModel({
      ...
      steps: [
        { line: 1, stack: [{ name: 'main', slots: [{ name: 'x', value: '(대기)', bad: true }] }], heap: {}, note: '<code>doubleB(10)</code> 호출 직전.', engine: '<b>호출 직전</b> — doubleB 프레임 없음. 화살표 함수도 <b>함수 객체</b>(힙)이고 const 슬롯이 그걸 가리킨다. 인수 10은 <b>SMI</b>.' },
        { line: 0, stack: [{ name: 'main', slots: [{ name: 'x', value: '(대기)', bad: true }] }, { name: 'doubleB', slots: [{ name: 'n', value: '10' }] }], heap: {}, note: '화살표 함수도 부르면 <b>똑같이 프레임 push</b> — n=10. 표기만 짧을 뿐 동작은 같다.', engine: '<b>doubleB(10) 호출</b> → 화살표 함수도 보통 함수와 똑같이 새 <b>스택 프레임(실행 컨텍스트)</b> push, n=10(SMI 값복사). 표기만 짧을 뿐 실행 컨텍스트는 동일하다.' },
