  ...
  window.Lessons['5-1'] = function render(root) {
    ...
    root.querySelector('[data-m="mem"]').append(MemoryModel({
      ...
      steps: [
        ...
        { line: 1, stack: [{ name: 'main', slots: [{ name: 'a', value: '(대기)', bad: true }] }], heap: {}, note: '<b>⑤ 반환 — 값 내보내고 프레임 pop</b> — <code>return</code>이 <b>11000을 프레임 밖으로</b> 내보내고 withTax를 <b>즉시 끝낸다</b>(프레임 pop, 지역 price 사라짐). 반환값 11000은 <b>호출한 자리로</b> 건네지는 중 — a는 <b>아직 대기</b>.', engine: '<b>return</b> → 결과 11000이 <b>레지스터</b>에 실려 호출자에게 전달되고, withTax 프레임은 <b>pop(즉시 회수)</b> — price 슬롯 소멸. 대입은 아직 안 됐다 — 호출자가 받아야 한다.' },
  ...
  window.Lessons['5-3'] = function render(root) {
    ...
    root.querySelector('[data-m="mem"]').append(MemoryModel({
      ...
      steps: [
        ...
        { line: 1, stack: [{ name: 'main', slots: [] }], heap: {}, note: '<b>⑤ 반환 — 값 내보내고 프레임 pop</b> — <code>return</code>이 <b>"안녕, 민지 님!"</b>을 프레임 밖으로 내보내고 greet를 <b>즉시 끝낸다</b>(프레임 pop, 지역 name 사라짐). 반환값은 <b>호출한 자리(print의 인자)로</b> 건네지는 중.', engine: '<b>return</b> → 이어붙인 새 문자열의 <b>포인터</b>가 레지스터로 넘어가고 greet 프레임은 <b>pop</b>. 지역 슬롯 name은 사라진다(가리키던 "민지" 본체는 아무도 안 쓰면 추후 GC).' },
  ...
  window.Lessons['5-4'] = function render(root) {
    ...
    root.querySelector('[data-m="mem"]').append(MemoryModel({
      ...
      steps: [
        ...
        { line: 1, stack: [{ name: 'main', slots: [{ name: 'r', value: '(대기)', bad: true }] }], heap: {}, note: '<b>⑤ 반환 — 값 내보내고 프레임 pop</b> — <code>return x*2</code>가 <b>10을 프레임 밖으로</b> 내보내고 double을 <b>즉시 끝낸다</b>(프레임 pop, 지역 x 사라짐). 반환값 10은 <b>호출한 자리로</b> 가는 중 — r은 <b>아직 대기</b>.', engine: '<b>return 10</b> → 값이 <b>레지스터</b>에 실려 호출자에게 전달되고 double 프레임은 <b>pop(즉시 회수)</b> — x 슬롯 소멸. (return이 없었다면 싱글턴 <b>undefined</b>가 넘어갔을 것.)' },
        { line: 3, stack: [{ name: 'main', slots: [{ name: 'r', value: '10' }] }], heap: {}, note: '<b>⑥ 호출부 대입</b> — 호출한 자리 <code>let r = …</code>가 반환값 <b>10</b>을 받아 <b>r 셀에 담는다</b> ✔. <b>반환(⑤)과 대입(⑥)은 별개의 두 단계</b>. (return이 없었다면 r엔 <b>undefined</b>가 담겼을 것.)', engine: '레지스터의 반환값 10이 r 슬롯에 안착(SMI). 프레임은 이미 없다.' },
      ],
    }))
    wireGoto(root)
  }
  ...
  window.Lessons['5-5'] = function render(root) {
    ...
    root.querySelector('[data-m="f1"]').append(MemoryModel({
      ...
      steps: [
        ...
        { line: 1, stack: [{ name: 'main', slots: [{ name: 'sum', value: '(대기)', bad: true }] }], heap: {}, note: '<b>⑤ 반환 — 값 내보내고 프레임 pop</b> — <code>return a+b</code>가 <b>7을 프레임 밖으로</b> 내보내고 add를 <b>즉시 끝낸다</b>(프레임 pop, 지역 a·b 사라짐). 반환값 7은 <b>호출한 자리로</b> 가는 중 — sum은 <b>아직 대기</b>.', engine: '<b>return 7</b> → 값이 <b>레지스터</b>에 실려 호출자에게 전달되고 add 프레임은 <b>pop(즉시 회수)</b> — a·b 슬롯 소멸.' },
    ...
    root.querySelector('[data-m="f2"]').append(MemoryModel({
      ...
      steps: [
        ...
        { line: 2, stack: [{ name: 'main', slots: [{ name: 'r', value: '(대기)', bad: true }] }], heap: {}, note: '<b>⑤ 반환 — 자동 undefined, 프레임 pop</b> — 끝까지 <code>return</code>이 없으면 함수는 <b>자동으로 undefined</b>를 내보내고(에러가 아니라 <b>정상적인 "값 없음"</b>) shout를 끝낸다(프레임 pop, 지역 x·big 사라짐). 이 undefined가 <b>호출한 자리로</b> 가는 중 — r은 <b>아직 대기</b>.', engine: '끝까지 return이 없으면 엔진이 <b>싱글턴 undefined</b>를 반환값으로 넘긴다(에러 아님). shout 프레임은 <b>pop</b> → 지역 x·big 슬롯 즉시 소멸.' },
